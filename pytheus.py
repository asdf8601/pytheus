...
def read_cookie(cookie_file: Path) -> str:
    ...
    rprint(
        "[bold red]Please create the file and add the cookie value to it.[/bold red]"
    )
    raise typer.Abort()
...
def query_prometheus(
    start: str = typer.Option(
        ..., help="Start date in YYYY-MM-DD (hh:mm:ss) format UTC"
    ),
    end: str = typer.Option(
        ..., help="End date in YYYY-MM-DD  (hh:mm:ss)format UTC"
    ),
    step: str = typer.Option("3600", help="Step size in seconds"),
    query: str = typer.Option(..., help="Prometheus query"),
    route: str = typer.Option(
        "query_range", help="Route to query, either 'query' or 'query_range'"
    ),
    cookie: Path = typer.Option(
        COOKIE_FILE, help="File which contains the Cookie value for the header"
    ),
    output: Path = typer.Option(
        None,
        help="Output file to save the results. Prints to stdout if not provided.",
    ),
    ...
    if response.ok:
        ...
        if output:
            ...
            if not silent:
                rprint(
                    f"[bold green]Query successful! Results saved to: {output}[/bold green]"
                )
        else:
            if not silent:
        ...
        rprint(
            f"[bold red]Query failed with status code: {response.status_code}[/bold red]"
        )
